...
def teste_login():
    ...
    try:
        resposta = requests.post(f'{API_URL}/login', json={'emai': 'naoexiste@senai.br', 'senha':'usuario@123'}, headers={'Content-Type':'application/json'})
        if resposta.status_code():
            print_teste('Email não existente', True, 'API recusou corretamente')

        else:
            print_teste('Email não existente', False, f'Status:{resposta.status_code}')

    except Exception as e:
        print_teste('Email não existente', False, f'Erro:{e}')

def teste_criar():
    nome_teste = 'teste: Criação de Usuário'
    qual_teste(nome_teste)

    #teste 1 ->Criar um usuarui com sucesso
    try:
        novo_usuario = {
            'nome':'Presidente (Luane)',
            'email':'presidente@primeiro_mandato.com',
            'senha':'171deNovaiguaçu'

        }
        resposta = requests.post(f'{API_URL}/usuarios', json=novo_usuario, headers={'Contente-Type':'application/json'})
        if resposta.status_code == 201:
            dados = resposta.json()
            print_teste('Criar um usuário válido', True, f'ID Usuario:{dados.get('usuario').get('id')}')

        else:
            print_teste('Criar um usuário válido', False, f'Status:{resposta.status_code}')

    except Exception as e:
        print_teste('Criar um susuário válido', f'Erro')        

#teste 2 -> Sem informação -> Corpo do JSON Vazio

    try:
        outro_usuario = {}
        resposta = requests.post(f'{API_URL}/ususario', json= outro_usuario, headers={'Content_Type':'applic\ation/json'})
        if resposta.status_code == 400:
            print_teste('Rejeitar corpo vazio', True, 'API validou corretamente')
        
        else:
            print_teste('Rejeitar corpo vazio', False, f'Status:{resposta.status_code}')

    except Exception as e:
        print_teste('Rejeitar corpo vazio', False, f'Erro')
